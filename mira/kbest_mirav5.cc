...
void CuttingPlane(vector<shared_ptr<HypothesisInfo> >* cur_c, bool* again, vector<shared_ptr<HypothesisInfo> >& all_hyp, vector<weight_t> dense_weights)
{
  ...
  if(no_reweight)
  {
    ...
    for(int u=0;u!=all_hyp.size();u++)
    {
      ...
      //if (PRINT_LIST) LOG(INFO) << all_hyp[u]->mt_metric << " H:" << all_hyp[u]->hope << " S:" << t_score;
    ...
    for(int u=0;u!=all_hyp.size();u++)
    {
      ...
      //if (PRINT_LIST) LOG(INFO) << all_hyp[u]->mt_metric << " H:" << all_hyp[u]->hope << " F:" << all_hyp[u]->fear;
  ...
  if(DEBUG_CUT) LOG(INFO) << "Max Fear in constraint set " << max_fear_in_set->fear;
...
//         else if(pseudo_doc)
//         {
//           //LOG(INFO) << "CORP:" << corpus_bleu_score << " NEW:" << sent_stats->ComputeScore() << " sentscore:" << sentscore;
...
//         else
//         {
//           //LOG(INFO) << "Using sentence-level approximation - PASS - " << boost::lexical_cast<std::string>(cur_pass);
...
//       //LOG(INFO) << TD::GetString(d->yield) << " ||| " << d->score << " ||| " << sentscore << " " << approx_sentscore;

//       if (i < update_list_size){
// 	if (i == 0) //take cur best and add its bleu statistics counts to the pseudo-doc
//         {  }
// 	if(PRINT_LIST)LOG(INFO) << TD::GetString(d->yield) << " ||| " << d->score << " ||| " << sentscore;
...
//     if(PRINT_LIST){  LOG(INFO) << "Sorting "; for(int u=0;u!=all_hyp.size();u++)	LOG(INFO) << all_hyp[u]->mt_metric << " " << all_hyp[u]->features.dot(dense_weights_g); }
...
//     if(PRINT_LIST) { LOG(INFO) << "GOOD";  for(int u=0;u!=cur_good.size();u++) LOG(INFO) << cur_good[u]->mt_metric << " " << cur_good[u]->hope;}
//     /*    if (!cur_oracle) {      cur_oracle = cur_good[0];
//           LOG(INFO) << "Set oracle " << cur_oracle->hope << " " << cur_oracle->fear << " " << cur_oracle->mt_metric;      }
//           else      {
//           LOG(INFO) << "Stay oracle " << cur_oracle->hope << " " << cur_oracle->fear << " " << cur_oracle->mt_metric;      }    */
...
//     if(PRINT_LIST){ LOG(INFO)<< "BAD"<<endl; for(int u=0;u!=cur_bad.size();u++) LOG(INFO) << cur_bad[u]->mt_metric << " H:" << cur_bad[u]->hope << " F:" << cur_bad[u]->fear;}
...
void UpdateOracles(int sent_id, double *corpus_src_length, const ScorerP &sent_scorer,
  ...
  if (PRINT_LIST) {
    ...
    for (int u = 0; u != all_hyps->size(); u++)
      LOG(INFO) << all_hyps->at(u)->mt_metric << " " << all_hyps->at(u)->features.dot(dense_weights_g);
  }
  ...
  if (hope_select == 1) {
    ...
    for (int u = 0; u != all_hyps->size(); u++) {
      ...
      if (PRINT_LIST)
        LOG(INFO) << all_hyps->at(u)->mt_metric << " H:" << all_hyps->at(u)->hope << " S:" << t_score;
    }
    // sort hyps by hope score
  ...
  if (fear_select == 1) {
    ...
    for (int u = 0; u != all_hyps->size(); u++) {
      ...
      if (PRINT_LIST) LOG(INFO) << all_hyps->at(u)->mt_metric << " H:" << all_hyps->at(u)->hope << " F:" << all_hyps->at(u)->fear;
  ...
  if (PRINT_LIST) {
    ...
    for (int u = 0; u != cur_bad.size(); u++)
      LOG(INFO) << cur_bad[u]->mt_metric << " H:" << cur_bad[u]->hope << " F:" << cur_bad[u]->fear;
  }
...
void ToDecoder(const InputRecord &input, const SparseVector<weight_t> &delta, Messenger *m) {
  for (SparseVector<weight_t>::const_iterator dit = delta.begin();
    ...
    VLOG(1) << "[mira weight update] " << FD::Convert(dit->first) << " " << weight.weight
               << " = " << hex << weight.repr;
  }
...
int main(int argc, char** argv) {
  ...
  // LOG(INFO) << "Loaded " << ds.size() << " references for scoring with " << metric_name;
  ...
  */
  ...
  // if (!SILENT) LOG(INFO) << "Reading input from " << ((input == "-") ? "STDIN" : input.c_str());
  ReadFile in_read("-");
  ...
  tot.clear();
  tot += lambdas;
  VLOG(2) << "PASS " << cur_pass << " " << "LAMBDAS: " << lambdas;
  ...
  while(*in) {
    ...
    if (optimizer == 4) { //single dual coordinate update, cur_good selected on BLEU score only (not model+BLEU)
      ...
      LOG(INFO) <<"Optimizing with " << cur_constraint.size() << " constraints";
      ...
      while (optimize_again)
      {
        ...
        if(optimize_again)
        {
          ...
          while (iter < smo_iter)
          {
            ...
            if(cur_pair.empty()){iter=MAX_SMO; LOG(INFO) << "Undefined pair "; continue;} //pair is undefined so we are done with this smo
          ...
          //     if(!no_reweight)
          //       {
          //         if(DEBUG_SMO) LOG(INFO)<< "Decoding with new weights -- now orac are " << oracles[cur_sent].good.size();
      ...
      for(int u=0;u!=cur_constraint.size();u++)
      {
        LOG(INFO) << cur_constraint[u]->alpha << " " << cur_constraint[u]->hope << " " << cur_constraint[u]->fear;
